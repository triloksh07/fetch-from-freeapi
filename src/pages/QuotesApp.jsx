...
export default function QuotesApp() {
  ...
  // State for the currently displayed random quote
  const [activeRandomJoke, setActiveRandomJoke] = useState(null);

  // Function to pick a random quote from our cached array
  const pickRandomJoke = useCallback((quotesArray) => {
    if (!quotesArray || quotesArray.length === 0) return;
    const randomIndex = Math.floor(Math.random() * quotesArray.length);
    setActiveRandomJoke(quotesArray[randomIndex]);
  }, []);
